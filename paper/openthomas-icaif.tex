...
selectivity or a better probability. A preliminary 21-day replay (968 settled
markets) found a naked model-vs-market rule losing ($-$\$0.007/contract, 425
trades) and a market-prior blend positive ($+$\$0.028, 184 trades); the
pre-registered replay over \tbd{E1} settled days
\pend{E1}{confirms / rejects} that selectivity is the edge. We also describe a
self-improvement loop whose safety comes from keeping the evaluator off the
optimizer's edit surface; its empirical record is reported as it accrues. All
data, frozen replay rows, and results are published daily.
\end{abstract}
...
lines of Python, 212 tests) agent that operationalizes the harness thesis on
...
\paragraph{Empirical status.} The architecture is shipped; its record is thin.
Over \tbd{E8} weeks of daily meta-cycles the loop has recorded
\tbd{E8} promotions, \tbd{E8} rollbacks and \tbd{E8} Brier vetoes
(Table~\ref{tab:lineage}); the gate's four anti-gaming rules are demonstrated
as adversarial test cases. We make no claim that the loop has improved trading
until the lineage says so out of sample. Improvement axes are unified as
...
Table~\ref{tab:ablation} \tbd{E1}. Conclusion: \pend{E1}{the control's interval
[excludes / includes] zero, so the edge is [selectivity / the blend].}
...
\midrule
Naked model vs market & \tbd{} & \tbd{} & \tbd{} & \tbd{} \\
\quad $+$ learned station bias & \tbd{} & \tbd{} & \tbd{} & \tbd{} \\
\quad\quad $+$ market blend (prod.) & \tbd{} & \tbd{} & \tbd{} & \tbd{} \\
Control: no blend, bar raised & \tbd{} & \tbd{} & \tbd{} & \tbd{} \\
\bottomrule
\end{tabular}
\caption{E1 on the frozen dataset \tbd{E1}, \tbd{days} settlement days,
\tbd{rows} rows. CI: day-block bootstrap. maxDD: of the daily PnL path, \$ per
one contract per trade.}
\label{tab:ablation}
\end{table}

\paragraph{E2--E3: robustness.} \tbd{E2} \emph{(grid summary, which cells are positive)}. \tbd{E3} \emph{(PnL/ct at each snapshot hour vs the E1 half-width)}.

\paragraph{E4: does the LLM add anything?} \tbd{E4} \emph{(table of $\delta$ vs n, PnL/ct, Brier; baseline Brier)}. \pend{E4}{The bounded adjustment [improves /
does not change / worsens] Brier and [adds / does not add] expectancy; the
production clamp $\pm0.15$ [is / is not] the best of the sweep.}

\paragraph{E5: against the official guidance.} \tbd{E5} \emph{(consensus vs NBM vs GFS MOS on the paired rows: Brier of the baseline probability and PnL/ct of the same rule)}. \pend{E5}{The seven-model consensus with learned bias [beats / ties /
loses to] NBM.}

\paragraph{E6: where the edge lives.} \tbd{E6} \emph{(per-station and per-week table; fraction of stations/weeks positive)}.

\paragraph{E7: the harness, scored.} \tbd{E7} \emph{(final bankroll, return, max drawdown and binding-constraint counts for flat / Kelly / Kelly+caps / Kelly+caps+kill on a \$1{,)}000 paper bankroll}. \pend{E7}{Caps [reduce /
do not reduce] drawdown at [small / large] cost in return; the kill-switch
[fired / did not fire] in the window.}

\paragraph{E8--E9.} Table~\ref{tab:lineage} \tbd{E8}; station bias halves
\tbd{E9}.
...
\toprule
\textbf{gen} & \textbf{parent} & \textbf{operator} & \textbf{held-in} & \textbf{held-out} & \textbf{decision} \\
\midrule
0 & --- & operator & \tbd{} & \tbd{} & baseline \\
\tbd{} & \tbd{} & \tbd{} & \tbd{} & \tbd{} & \tbd{} \\
\bottomrule
\end{tabular}
\caption{E8: self-improvement lineage as exported by the loop
(\tbd{E8} meta-cycles). Empty until the loop has run long enough to
say anything.}
\label{tab:lineage}
\end{table}
...
thesis. \pend{E1}{On the data so far the edge is [selectivity / the blend].}
We report it in public, losses included, and invite the community to check it.
...
\bottomrule
\end{tabular}
\caption{Everything needed to regenerate the tables from the published files.}
\end{table}
