...
decision rules, written before the data; every number is filled from a daily
pipeline over a public, growing dataset and is regenerated as the window rolls
forward. The first run contradicted the thesis, and the thesis was rewritten,
not the data. Any cell that reads \tbd{} is awaiting the next run.}}\par\medskip}
...
failure is a \emph{harness} problem more than a forecasting one, and test that
claim where it is measurable: daily station-temperature markets on Kalshi and
Polymarket, which settle every day against the U.S.\ National Weather Service
report for a named station. \textbf{OpenThomas}, an open-source agent, prices
each strike from a seven-model NWP consensus with a per-station bias learned
from leak-free hindcasts, lets a language model make a \emph{bounded}
adjustment, blends with the market prior, and sizes with fractional Kelly under
risk limits the model cannot override. Our main contribution is methodological:
a leak-free replay on real order books with explicit windows, frozen and hashed
datasets, local-time decision snapshots, official NBM/MOS baselines, day-block
bootstrap error bars, and a \emph{pre-registered} ablation whose control
decides whether the edge is selectivity or a better probability. A preliminary
21-day replay had found a market-prior blend positive ($+$\$0.028/contract).
The pre-registered replay over 64 settlement days (3{,}349 markets) does not:
every setting of the baseline-only rule loses ($-$\$0.034/contract, 95\% CI
$[-0.069, +0.002]$), the blend and a raised edge bar are the same lever, and
the consensus with learned bias ties the NWS National Blend of Models (Brier
0.163 vs.\ 0.163). The bounded LLM adjustment improves Brier slightly and the
clamp sits at the best of a sweep in which the unclamped model does worse. What
survives is the harness: the drawdown kill-switch is the one rail that
preserved capital under a losing rule, and the self-improvement gate refused
thin evidence and rolled one promotion back. We report the result that
contradicts our own thesis, and what it changes. All data, frozen replay rows,
and results are published daily.
\end{abstract}
...
dataset, and the conclusions follow the numbers; where they contradicted our
thesis, we changed the thesis.
...
with archived lead-1 guidance for every station) to 2026-09-10 at the time
of writing, rolling
...
property of one cell. Snapshot sensitivity: moving the low snapshot from local
00:00 to 01:00 or 02:00 changes production expectancy from $-0.034$ to $-0.032$
and $-0.035$; moving the high snapshot from 11:00 back to 09:00 changes it to
$-0.046$ $[-0.083, -0.010]$. The largest shift ($0.012$) is a third of the E1
half-width ($0.035$), so timing is not doing the work. The direction of the
09:00 result matters more than its size: at 09:00 the book knows \emph{less}
about the day and the model loses \emph{more}, so the model is not being
beaten by a book that already saw the morning; it is beaten by a book that
prices the same guidance better.

\paragraph{E4: does the LLM add anything?} Table~\ref{tab:llm}, on a
400-row deterministic stride sample of the frozen dataset (a reasoning model
costs about a minute per row). The bounded adjustment does not worsen
calibration: Brier improves monotonically from the baseline's $0.1706$ to
$0.1691$ at $\delta = 0.15$ and $0.25$, and the \emph{unclamped} model
($\delta=\infty$) gives back part of that ($0.1694$). The production clamp is
at the best of the sweep, which is the harness thesis in miniature: the model
helps a little when it may only nudge, and helps less when it may replace.
The effect is small ($-0.0015$ Brier, about $1\%$) and the PnL movement
($+0.009 \to +0.020$/contract on $\sim$78 trades) is well inside a $\pm 0.10$
interval; on this sample the baseline itself is slightly positive, which is
the sampling noise the full-dataset E1 warns about. The live paper run tells
a different story (Section~\ref{sec:live}): the live forecaster's Brier over
its 41 \emph{traded} markets is $0.43$ against a market Brier of
$0.10$--$0.19$ at the same stations. The two numbers are not the same
quantity (replay scores every row; live scores only the markets the agent
chose to trade, a selection biased toward its largest disagreements with the
market), but the gap says where the damage is done: not in the clamped
adjustment, but in the trade selection downstream of it.
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Clamp $\delta$} & \textbf{n} & \textbf{PnL/ct} & \textbf{95\% CI} & \textbf{Brier} \\
\midrule
0 (baseline, no LLM) & 79 & $+0.009$ & $[-0.087, +0.098]$ & 0.1706 \\
$\pm 0.05$ & 77 & $+0.016$ & $[-0.089, +0.121]$ & 0.1698 \\
$\pm 0.10$ & 77 & $+0.016$ & $[-0.089, +0.121]$ & 0.1694 \\
$\pm 0.15$ (production) & 78 & $+0.020$ & $[-0.082, +0.123]$ & 0.1691 \\
$\pm 0.25$ & 79 & $+0.019$ & $[-0.081, +0.121]$ & 0.1691 \\
$\infty$ (unclamped) & 79 & $+0.019$ & $[-0.081, +0.121]$ & 0.1694 \\
\bottomrule
\end{tabular}
\caption{E4: LLM-in-replay on 400 stride-sampled rows of the frozen dataset
(digest 7c3fb9f21ab727e3): a self-hosted DeepSeek reasoning model, temperature
0, one sample per row, prompt rebuilt from as-of data only; its probability is
clipped to baseline $\pm\delta$ and pushed through the production decision
rule. Brier is over all 400 rows; PnL over the trades the rule takes.}
\label{tab:llm}
\end{table}
...
adjustment that keeps doing on live inputs what it does on replay rows: E4
shows a clamped model improving calibration slightly and an unclamped one
giving part of it back, while the live forecaster's Brier says the loss is
made downstream, in which markets get traded. We report the result in public,
losses included, and invite the community to check it.
...
LLM (E4) & DeepSeek (self-hosted, OpenAI-compatible endpoint); temperature 0, single sample, prompt cached by exact text; 400-row deterministic stride sample \\
